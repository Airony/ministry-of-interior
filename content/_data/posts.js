const wordpressAPI =
  "http://localhost/wordpress/wp-json/wp/v2/posts?orderby=date&order=desc";
...
async function wpPostPages() {
  try {
    const res = await fetch(`${wordpressAPI}&_fields=id&pages=1`);
...
function cleanLink(str) {
  return str.replace("http://localhost/wordpress/", "").trim();
}
